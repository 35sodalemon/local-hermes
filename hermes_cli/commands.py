...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("topic", "启用或查看 Telegram DM 主题会话", "会话",
    ...
    CommandDef("save", "保存当前对话", "会话",
               cli_only=True),
    CommandDef("retry", "重发上一条消息（重新发送给 agent）", "会话"),
    CommandDef("undo", "移除最后一条用户/助手交互", "会话",
               args_hint="[N]"),
    CommandDef("title", "为当前会话设置标题", "会话",
               args_hint="[name]"),
    CommandDef("handoff", "将此会话转移到消息平台（Telegram、Discord 等）", "会话",
    ...
    CommandDef("deny", "拒绝待处理的危险命令", "会话",
               gateway_only=True),
    CommandDef("background", "在后台运行提示（别名 bg, btw）", "会话",
               aliases=("bg", "btw"), args_hint="<prompt>"),
    CommandDef("agents", "显示活跃的 agent 和运行中的任务", "会话",
               aliases=("tasks",)),
    CommandDef("queue", "将提示排队到下一轮（不中断当前处理）", "会话",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "在下次工具调用后注入消息（不中断）", "会话",
               args_hint="<prompt>"),
    CommandDef("goal", "设置一个持续目标，Hermes 会跨轮次工作直到达成", "会话",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("subgoal", "添加或管理当前目标的额外条件", "会话",
               args_hint="[text | remove N | clear]"),
    CommandDef("status", "显示会话信息", "会话"),
    CommandDef("whoami", "显示你的斜杠命令权限（管理员/用户）", "信息"),
    CommandDef("profile", "显示当前 profile 名称和主目录", "信息"),
    ...
    CommandDef("verbose", "循环工具进度显示：关 → 新工具 → 全部 → 详细", "配置", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("footer", "切换网关回复元数据页脚", "配置", args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("yolo", "切换 YOLO 模式（跳过所有危险命令审批）", "配置"),
    ...
    CommandDef("skin", "显示或更改显示主题/皮肤", "配置",
    ...
    CommandDef("busy", "控制 Hermes 工作时回车键的行为", "配置",
    ...
    CommandDef("memory", "查看待审批的记忆写入 / 切换审批开关", "工具与技能",
               args_hint="[pending|approve|reject|approval] [id|on|off]",
               subcommands=("pending", "approve", "reject", "approval")),
    CommandDef("bundles", "列出技能包（用 /<name> 作为多个技能的别名）", "工具与技能"),
    CommandDef("cron", "管理定时任务", "工具与技能",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("suggestions", "查看建议的自动化任务（接受/忽略）", "工具与技能",
               aliases=("suggest",), args_hint="[accept|dismiss N | catalog]",
               subcommands=("accept", "dismiss", "catalog", "clear")),
    CommandDef("blueprint", "从蓝图模板设置自动化任务", "工具与技能",
               aliases=("bp",), args_hint="[name] [slot=value ...]"),
    CommandDef("curator", "技能后台维护（状态/运行/置顶/归档/列表）", "工具与技能", args_hint="[subcommand]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore", "list-archived")),
    CommandDef("kanban", "多 profile 协作看板（任务/链接/评论）", "工具与技能", args_hint="[subcommand]",
    ...
    CommandDef("plugins", "列出已安装的插件及其状态", "工具与技能", cli_only=True),

    # Info
    CommandDef("commands", "浏览所有命令和技能（分页）", "信息",
               gateway_only=True, args_hint="[page]"),
    CommandDef("help", "显示可用命令", "信息"),
    CommandDef("restart", "优雅地重启网关（等待活跃运行完成后）", "会话",
               gateway_only=True),
    ...
    CommandDef("paste", "从剪贴板附加图片", "信息",
               cli_only=True),
    CommandDef("image", "附加本地图片文件用于下一条提示", "信息",
