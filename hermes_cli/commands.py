...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("new", "重置会话，开始新对话", "会话",
               aliases=("reset",), args_hint="[名称]"),
    CommandDef("topic", "启用或查看 Telegram DM 主题会话", "会话",
               gateway_only=True, args_hint="[off|help|会话ID]"),
    ...
    CommandDef("undo", "移除最后一组对话交换", "会话"),
    CommandDef("title", "为当前会话设置标题", "会话",
               args_hint="[名称]"),
    CommandDef("branch", "分支当前会话（探索不同方向）", "会话",
    ...
    CommandDef("verbose", "循环工具进度显示：关闭 -> 新工具 -> 全部 -> 详细",
               "Configuration", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("footer", "切换网关回复元数据页脚",
    ...
    CommandDef("fast", "切换快速模式 — OpenAI 优先处理 / Anthropic 快速模式", "配置",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "显示或更改显示主题", "配置",
    ...
    CommandDef("curator", "技能后台维护（状态/运行/归档）",
               "Tools & Skills", args_hint="[子命令]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore")),
    CommandDef("kanban", "多配置协作看板（任务/链接/评论）",
               "Tools & Skills", args_hint="[子命令]",
    ...
    CommandDef("reload", "重新加载 .env 环境变量到当前会话", "工具与技能",
               cli_only=True),
    CommandDef("reload-mcp", "从配置重新加载 MCP 服务器", "工具与技能",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "重新扫描 ~/.hermes/skills/ 以发现新安装或已删除的技能",
               "Tools & Skills", aliases=("reload_skills",)),
    CommandDef("browser", "通过 CDP 连接浏览器工具到你的 Chrome", "工具与技能",
    ...
    CommandDef("restart", "优雅重启网关（等待活跃任务完成后）", "会话",
               gateway_only=True),
    ...
    CommandDef("copy", "复制最后一条助手回复到剪贴板", "信息",
               cli_only=True, args_hint="[number]"),
    CommandDef("paste", "附加剪贴板中的图片", "信息",
               cli_only=True),
    CommandDef("image", "附加本地图片文件到下一条提示", "信息",
