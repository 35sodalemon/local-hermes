    ...
    def __init__(
        ...
        if self.api_mode == "anthropic_messages":
            ...
            if _is_bedrock_anthropic:
                ...
                self._client_kwargs = {}
                if not self.quiet_mode:
                    print(f"🤖 AI Agent 已初始化，模型：{self.model}（Anthropic 原生）")
            ...
            try:
                ...
                if not self.quiet_mode:
                    print(f"🤖 AI Agent 已初始化，模型：{self.model}")
                    if base_url:
                        print(f"🔗 使用自定义 base URL：{base_url}")
                    # Always show API key info (masked) for debugging auth issues
                    key_used = client_kwargs.get("api_key", "none")
                    if key_used and key_used != "dummy-key" and len(key_used) > 12:
                        print(f"🔑 使用 API key：{key_used[:8]}...{key_used[-4:]}")
                    else:
        ...
        if self._fallback_chain and not self.quiet_mode:
            if len(self._fallback_chain) == 1:
                ...
                print(f"🔄 备用链（{len(self._fallback_chain)} 个供应商）：" +
        ...
        if self.tools:
            ...
            if not self.quiet_mode:
                ...
                if enabled_toolsets:
                    print(f"   ✅ 已启用的工具集：{', '.join(enabled_toolsets)}")
                if disabled_toolsets:
                    print(f"   ❌ 已禁用的工具集：{', '.join(disabled_toolsets)}")
        elif not self.quiet_mode:
            print("🛠️  未加载任何工具（全部被过滤或不可用）")
        ...
        if self.ephemeral_system_prompt and not self.quiet_mode:
            ...
            print(f"🔒 临时系统提示：'{prompt_preview}'（不保存到轨迹中）")
        ...
        if self._use_prompt_caching and not self.quiet_mode:
            ...
            print(f"💾 Prompt 缓存：已启用（{source}，{self._cache_ttl} TTL）")
